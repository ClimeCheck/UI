import SensorReading from "./sensorReadings";
import SensorTopBar from "./sensorTopBAr";
import { sensorData } from "./sensorData";
import Guage from "./sensorReader";

export default function SensorComponent() {
  return (
    <div className="container font-Grotesk">
      <div className="flex flex-col">
        <SensorTopBar
          datastreams={"1234"}
          status={"Active"}
          earnings={"1.432"}
        />
        <div className="grid grid-cols-3">
          <div className="col-span-2">
            <div className="flex flex-col">
              <Guage value={231} />
              <div>
                <p className="font-bold text-[#74BF44] my-4 text-xl text-center">
                  Air Quality Index
                </p>
                <p className="text-black text-center">Sensor type: Indoor</p>
              </div>
              <div>
                <p className="text-[#777777] my-2">
                  <span className="border rounded-full italic">i</span> Color
                  indication
                </p>
                <div className="flex flex-row gap-2">
                  <div>
                    <p className="text-[#800000]">
                      <span className="bg-[#800000] p-1"></span>&larr;
                    </p>{" "}
                    Danger
                  </div>
                  <div>
                    <p className="text-[#F46B08]">
                      <span className="bg-[#F46B08] p-1"></span>&larr;
                    </p>{" "}
                    Warning
                  </div>
                  <div>
                    <p className="text-[#008000]">
                      <span className="bg-[#008000] p-1"></span>&larr;
                    </p>{" "}
                    Good
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-span-1">
            <SensorReading sensordata={sensorData} />
          </div>
        </div>
      </div>
    </div>
  );
}
